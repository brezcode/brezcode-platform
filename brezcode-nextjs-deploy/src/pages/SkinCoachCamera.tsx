...
interface SkinCoachCameraProps {
  condition?: 'acne' | 'wrinkles' | 'dark-spots' | 'general';
  skipQuiz?: boolean;
  backRoute?: string;
}

export default function SkinCoachCamera({ 
  condition = 'general',
  skipQuiz = false,
  backRoute = '/skincoach/quiz'
}: SkinCoachCameraProps = {}) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      formData.append('image', blob, `${condition}_analysis.jpg`);
      
      // Create quiz data based on condition and skipQuiz setting
      const quizData = skipQuiz ? getConditionSpecificQuizData(condition) : getStoredQuizData();
      ...
      localStorage.setItem('analysisCondition', condition);
      
      setAnalysisStep('complete');
      
      // Navigate to appropriate results page
      setTimeout(() => {
        const resultsRoute = condition === 'acne' ? '/skincoach/results-acne' : '/skincoach/results';
        setLocation(resultsRoute);
      }, 2000);
  ...
  // Helper functions for different conditions
  const getConditionSpecificQuizData = (condition: string) => {
    const baseQuizData = {
      age: '26-35',
      gender: 'prefer-not-to-say',
      skinType: 'combination',
      goals: [`reduce-${condition}`],
      routine: 'basic',
      sunExposure: 'medium',
      lifestyle: ['normal'],
      budget: 'moderate',
      previousTreatments: '',
      allergies: '',
      additionalNotes: `${condition}-focused analysis using specialized AI models`
    };
  ...
  const getStoredQuizData = () => {
    try {
      const stored = localStorage.getItem('skinCoachQuizData');
      return stored ? JSON.parse(stored) : getConditionSpecificQuizData(condition);
    } catch {
      return getConditionSpecificQuizData(condition);
    }
  };
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <h2 className="text-3xl font-bold text-gray-900 mb-2">
          {condition === 'acne' ? 'Acne Analysis Photo' : 
           condition === 'wrinkles' ? 'Anti-Aging Analysis Photo' :
           condition === 'dark-spots' ? 'Pigmentation Analysis Photo' :
           'Take Your Skin Photo'}
        </h2>
        <p className="text-gray-600">
          {condition === 'acne' ? 'Capture the area with acne for AI-powered analysis using Skyn technology' :
           condition === 'wrinkles' ? 'Focus on areas with fine lines and wrinkles for age assessment' :
           condition === 'dark-spots' ? 'Position dark spots and pigmentation clearly in the frame' :
           'Position your face in the frame for the best analysis'}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-center gap-4">
          ...
          >
            ...
            {condition === 'acne' ? 'Capture Acne Photo' :
             condition === 'wrinkles' ? 'Capture Anti-Aging Photo' :
             condition === 'dark-spots' ? 'Capture Pigmentation Photo' :
             'Capture Photo'}
          </Button>
  ...
  const renderAnalysisInterface = () => (
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600">Our AI is examining your photo for skin insights</p>
      </div>

      <div className="space-y-4">
        {[
          { step: 'Detecting facial features', status: 'complete' },
          { step: 'Analyzing skin texture', status: 'complete' },
          { step: 'Identifying skin concerns', status: 'active' },
          { step: 'Calculating skin scores', status: 'pending' },
      ...
      <div className="bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200 rounded-lg p-6">
        ...
        <p className="text-gray-700 text-sm leading-relaxed">
          We're using advanced computer vision and machine learning models to analyze over 50 different skin characteristics. 
          This process typically takes 30-60 seconds for the most accurate results.
        </p>
      </div>
    </div>
  );
  ...
  const renderCompleteInterface = () => (
    <div className="space-y-6 text-center">
      ...
      <p className="text-gray-600 text-lg">
        Your comprehensive skin report is ready. Redirecting you to results...
      </p>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              ...
              {skipQuiz ? 'Back to Home' : 'Back to Quiz'}
            </Button>
            <Badge className="px-4 py-2 bg-gradient-to-r from-pink-500 to-purple-500 text-white">
              {condition === 'acne' ? 'Acne Analysis with Skyn AI' :
               condition === 'wrinkles' ? 'Anti-Aging Analysis' :
               condition === 'dark-spots' ? 'Pigmentation Analysis' :
               skipQuiz ? 'AI Skin Analysis' : 'Step 2: Photo Analysis'}
            </Badge>
          </div>
        </div>
